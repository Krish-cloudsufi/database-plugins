...
Feature: PostgreSQL - Verify PostgreSQL plugin data transfer with macro arguments
  ...
  Scenario: To verify data is getting transferred from PostgreSQL to PostgreSQL successfully when macro enabled
    ...
    Then Enter runtime argument value "splitByField" for key "PostgreSQLSplitByColumn"
    Then Enter runtime argument value "numberOfSplits" for key "PostgreSQLNumSplits"
    Then Enter runtime argument value "selectQuery" for key "PostgreSQLImportQuery"
